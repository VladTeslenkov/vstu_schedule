...
***В случае, если nginx нужен в контейнере*** `docker compose --profile full up -d --build` (подтянет nginx автоматически). Только при отключении тогда не забудьте так же: `docker compose --profile full down`
