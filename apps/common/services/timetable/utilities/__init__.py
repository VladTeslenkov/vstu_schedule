...
def get_name_from_month_number(month_number: int | list[int]) -> str | list[str | None] | None:
    ...
    if isinstance(month_number, list):
        return [MONTH_NAMES[i - 1] if i >= 1 and i <= 12 else None for i in month_number]
...
def is_similar_events(first_event: Event, second_event: Event) -> bool:
    ...
    return (
        first_event.subject_override == second_event.subject_override
        and list(first_event.get_groups()) == list(second_event.get_groups())
        and list(first_event.get_teachers()) == list(second_event.get_teachers())
        and list(first_event.places_override.all()) == list(second_event.places_override.all())
    )
