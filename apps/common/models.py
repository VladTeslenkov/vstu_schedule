...
class CommonModel(models.Model):
    ...
    @classmethod
    def last_modified_record(cls) -> Self | None:
...
class EventPlace(CommonModel):
    building = models.CharField(
        blank=True, default="", db_default="", max_length=128, verbose_name="Корпус"
    )
...
class AbstractDay(CommonModel):
    day_number = models.IntegerField(
        verbose_name="Смещение от начала повторяющгося фрагмента (пн. первой недели)"
    )
...
class Department(CommonModel):
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, verbose_name="Учреждение"
    )
...
class ScheduleTemplate(CommonModel):
    metadata = models.ForeignKey(
        ...
        verbose_name="Факультет, обучение",
    )
    ...
    aligned_by_week_day = models.IntegerField(
        verbose_name="Выравнивание относительно дня недели (null=0, пн=1, ...)"
    )
    department = models.ForeignKey(
        Department, null=True, on_delete=models.SET_NULL, verbose_name="Подразделение"
    )
...
class Schedule(CommonModel):
    ...
    metadata = models.ForeignKey(
        ScheduleMetadata, null=True, on_delete=models.PROTECT, verbose_name="Курс, семестр, год"
    )
    ...
    starting_day_number = models.ForeignKey(
        AbstractDay,
        null=True,
        on_delete=models.PROTECT,
        verbose_name="Номер дня начала первого повторяющегося цикла",
    )
    schedule_template = models.ForeignKey(
        ScheduleTemplate, null=True, on_delete=models.PROTECT, verbose_name="Шаблон расписания"
    )
    ...
    def __repr__(self):
        return (
            f"{self.Status(self.status).label}, {self.schedule_template.metadata}, {self.metadata}"
        )
    ...
    def first_event(self):
        ...
        return (
            events.annotate(min_date=models.Min("holdings__date")).order_by("min_date").first()
        )  ####
    ...
    def last_event(self):
        ...
        return (
            events.annotate(max_date=models.Max("holdings__date")).order_by("-max_date").first()
        )  ######
...
class AbstractEventChanges(CommonModel):
    ...
    is_exported = models.BooleanField(
        verbose_name="Запланированное событие экспортировано", default=False
    )
    origin_teachers = models.TextField(
        null=True, blank=True, default="", verbose_name="Изначальные участники"
    )
    origin_places = models.TextField(
        null=True, blank=True, default="", verbose_name="Изначальные места"
    )
    origin_holds_on_date = models.TextField(
        null=True, blank=True, default="", verbose_name="Изначальные заданный день"
    )
    origin_kind = models.TextField(
        null=True, blank=True, default="", verbose_name="Изначальный тип"
    )
    final_teachers = models.TextField(
        null=True, blank=True, default="", verbose_name="Участники после изменений"
    )
    final_places = models.TextField(
        null=True, blank=True, default="", verbose_name="Места после изменений"
    )
    final_date_time = models.TextField(
        null=True, blank=True, default="", verbose_name="Дата и учебный час после изменений"
    )
    final_holds_on_date = models.TextField(
        null=True, blank=True, default="", verbose_name="Заданный день после изменений"
    )
    final_kind = models.TextField(
        null=True, blank=True, default="", verbose_name="Тип после изменений"
    )
    ...
    def get_export_data(self) -> list[list[str]]:
        ...
        export_data_base = [
            self.datemodified.strftime("%Y-%m-%d %H:%M:%S"),
            self.group,
            self.date_time,
            self.subject,
        ]
        ...
        if self.is_deleted:
            ...
            if self.final_teachers:
                export_data.append(
                    [*export_data_base, "ПРЕПОДАВАТЕЛЬ", self.origin_teachers, self.final_teachers]
                )

            if self.final_places:
                export_data.append(
                    [*export_data_base, "АУДИТОРИЯ", self.origin_places, self.final_places]
                )

            if self.final_date_time:
                export_data.append(
                    [*export_data_base, "ДЕНЬ НЕДЕЛИ/УЧ. ЧАС", self.date_time, self.final_date_time]
                )
            ...
            if self.final_holds_on_date:
                export_data.append(
                    [
                        *export_data_base,
                        "ЯВНАЯ ДАТА",
                        self.origin_holds_on_date,
                        self.final_holds_on_date,
                    ]
                )
...
class AbstractEvent(CommonModel):
    kind = models.ForeignKey(
        EventKind, null=True, blank=True, default=None, on_delete=models.PROTECT, verbose_name="Тип"
    )
    ...
    abstract_day = models.ForeignKey(
        AbstractDay, on_delete=models.PROTECT, verbose_name="Абстрактный день"
    )
    ...
    holds_on_date = models.DateField(
        null=True, blank=True, verbose_name="Проводится только в заданный день"
    )
    ...
    def update_change_model(self):
        ...
        is_date_time_changed = (
            previous_ae.abstract_day != self.abstract_day or previous_ae.time_slot != self.time_slot
        )
...
class DayDateOverride(CommonModel):
    ...
    department = models.ForeignKey(
        Department, null=True, on_delete=models.CASCADE, verbose_name="Подразделение"
    )
...
class Event(CommonModel):
    ...
    date_override = models.ForeignKey(
        DayDateOverride,
        null=True,
        blank=True,
        editable=False,
        on_delete=models.SET_NULL,
        verbose_name="Перенос дня",
    )
    kind_override = models.ForeignKey(
        EventKind, null=True, blank=True, default=None, on_delete=models.PROTECT, verbose_name="Тип"
    )
    subject_override = models.ForeignKey(
        Subject, null=True, on_delete=models.PROTECT, verbose_name="Предмет"
    )
    ...
    time_slot_override = models.ForeignKey(
        TimeSlot, null=True, on_delete=models.PROTECT, verbose_name="Временной интервал"
    )
    abstract_event = models.ForeignKey(
        AbstractEvent,
        null=True,
        editable=False,
        on_delete=models.CASCADE,
    ...
    event_cancel = models.ForeignKey(
        EventCancel, null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Отмена события"
    )
    ...
    def get_teachers(self):
        return self.participants_override.filter(
            role__in=[EventParticipant.Role.TEACHER, EventParticipant.Role.ASSISTANT]
        )
    ...
    def check_date_interactions(self):
        ...
        if not self.date_override:
            ...
            if reader.get_found_models().exists():
                apply_day_date_override(
                    reader.get_found_models().first(), self, call_save_method=False
                )
