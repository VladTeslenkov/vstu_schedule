...
Этот проект необходим для предоставления нового, интерактивного расписания ВУЗа, на основе "сырых" Excel файлов, содержащих расписания занятий для набора групп. Поскольку, проект требует сразу нескольких компонентов (Redis, PostgreSQL, запуск Celery), его лучше запускать в Docker через compose.
...
- Данный npm проект (или, возможно в будущем, несколько проектов) должен собираться в standalone js, css файлы и легко подключаться к шаблону Django / или просто HTML файлу из папки `templates` (у конкретного приложения или в корне проекта)
...
2. Сложная функциональность бизнес-логики должна быть покрыта тестами `pytest` (но не непосредственное тестирование фрагментов библиотеки `vstu-xls`)
...
7. Код Python должен быть написан в соответствии с принципами PEP8, SOLID, KISS и с учетом современных возможностей языка в минимально поддерживаемой версии языка
...
10. Работоспособность существующих функций проекта не должна быть сломана в новом PR, если она существенно меняется - об этом нужно сообщить в тексте PR. Проверка работоспособности проекта после изменений необходимо
...
> Стоит отметить, что запуск проекта в локальном режиме по-прежнему возможен, но требует наличия доступной PostgreSQL базы локально или удалённо, и ручного запуска Celery или переменной `DISABLE_CELERY=1` в .env, если он нужен
