...
- Исправление текстов или сообщений в клиентской части должно выполняться через файлы переводов `django.po`. Нельзя хардкодить текст, который должен быть доступен для перевода.
...
## Дескрипторы Celery-задач

Celery-задачи объявляются внутри пакета `tasks` на уровне приложения. Если у
приложения есть такой пакет, задачи, которые должен находить Celery, должны быть
доступны из `tasks/__init__.py`.

Каждый пакет `tasks` должен содержать дескриптор `tasks.toml` рядом с
`__init__.py`. Дескриптор используется как источник названий задач в панели,
описаний, рекомендуемых таймаутов Celery и рекомендуемого расписания
периодического запуска. При старте выполняется лёгкая проверка: если проектная
Celery-задача зарегистрирована, но для неё нет дескриптора, в лог пишется
warning.

В качестве имени TOML-таблицы используется полное имя Celery-задачи в кавычках:

```toml
["panel.tasks.update_timetable"]
name = { ru = "Обновление расписания", en = "Timetable update" }
